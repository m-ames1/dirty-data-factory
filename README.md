...
about reproducing a specific prior run byte-for-byte (see "Generating the
clean data" below).
...
its seed flags. That's fine: reproducing a specific prior run isn't a goal
here, and there's no check anywhere expecting byte-for-byte matches.
