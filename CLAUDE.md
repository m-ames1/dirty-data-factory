...
Every regeneration is its own new batch; there is no check anywhere that
expects two runs to match byte-for-byte. Never hand-edit generated output in
`data/` — if the data needs to change, change the generation parameters/code
and regenerate.
...
- Synthea's generation isn't exactly reproducible run-to-run, even with the
  ...
  documented seed flags. This is expected, not a bug: every batch is its own
  new generation, and reproducing a specific prior batch isn't a goal — there
  is no CI check (or anything else) that expects it.
