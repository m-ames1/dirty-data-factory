...
from dirty_data_factory.csv_io import Table, load_all_tables, write_all_tables
...
def run(config: Config) -> RunResult:
    tables = load_all_tables(config.input_dir)

    manifest_path = (
        config.output_dir / ".manifest_withheld" / "manifest.jsonl"
        if config.withhold_manifest
        else config.output_dir / "manifest.jsonl"
    )
    ...
    write_all_tables(tables, config.output_dir)

    summary_path = config.output_dir / "manifest_summary.json"
